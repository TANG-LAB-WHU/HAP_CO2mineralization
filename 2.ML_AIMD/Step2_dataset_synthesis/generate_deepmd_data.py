...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Generate DeepMD dataset from CP2K outputs.")
    parser.add_argument(
        "--cp2k-source-dir",
        default="cp2k_run",
        help="Directory containing CP2K outputs for one structure",
    )
    parser.add_argument(
        "--output-dir",
        default="deepmd_data",
        help="Output DeepMD system directory",
    )
    parser.add_argument(
        "--project-name",
        default="",
        help="Optional project prefix (for example HAP_112). If empty, auto-detect.",
    )
    parser.add_argument(
        "--no-clean-output",
        action="store_true",
    ...
    cp2k_source_dir = pathlib.Path(args.cp2k_source_dir)
    output_dir = pathlib.Path(args.output_dir)

    print(f"Output directory: {output_dir}")
    print(f"CP2K source directory: {cp2k_source_dir}")

    # Auto-detect project name from files in cp2k_source_dir unless provided
    project_name = args.project_name if args.project_name else detect_project_name(cp2k_source_dir)
    print(f"Using project name: {project_name}")

    # Clean and recreate the output directory unless explicitly disabled
