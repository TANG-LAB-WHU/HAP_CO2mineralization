...
REM Using the Python 3.10-slim image that's already pulled
docker run --rm -v "%cd%:/workspace" -w /workspace docker.m.daocloud.io/library/python:3.10-slim bash -c "pip install --quiet -i https://pypi.tuna.tsinghua.edu.cn/simple -r requirements.txt && python generate_deepmd_data.py"
